import { build, Action, toInt, routeParamSelector, Collection, distinct, inArray } from '@caiu/library';
import { Store } from '@ngrx/store';
import { Observable, combineLatest } from 'rxjs';
import { map, distinctUntilChanged, mergeMap } from 'rxjs/operators';

import { Members, Member } from './members.model';
import { AccountMember } from '../accounts/accounts.model';
import { Group, GroupMember } from '../groups/groups.model';
import { GroupActions, GroupsActions } from '../groups/groups.reducer';
import { usersSelector, userIdSelector, currentUserIdSelector } from '../shared/selectors';
...
export function membersSelector(store: Store<any>, accountId = 0): Observable<Members> {
    ...
    return combineLatest(accountMembers$, groupMembers$, accountId$, userId$, Members.BuildMembers);
}

export function memberIdSelector(store: Store<any>, accountId = 0): Observable<string> {
    return membersSelector(store).pipe(
        map(members => members.activeId),
...
export function membersUserIdSelector(store: Store<any>): Observable<number> {
    return store.select('members').pipe(
...
export function accountIdMembersSelector(store: Store<any>, accountId = 0): Observable<AccountMember[]> {
    ...
    return accountId === 0 ? accountMembers$
        : accountMembers$.pipe(
            map(accountMembers => accountMembers.filter(member => member.accountId === accountId))
        );
}
...
export function accountMemberRolesSelector(store: Store<any>, accountId = 0, roles: number[] = []): Observable<AccountMember[]> {
    ...
    return members$.pipe(
        map(members => distinct(members.filter(x => inArray(roles, x.accountRoleId)), 'userId'))
    );
}
...
export function groupIdMembersSelector(store: Store<any>, groupId = 0, accountId = 0): Observable<GroupMember[]> {
    ...
    const members$ = groupId === 0 ? (accountId === 0 ? groupMembers$
        : groupMembers$.pipe(
            map(groupMembers => groupMembers.filter(member => member.accountId === accountId)))
    )
        : groupMembers$.pipe(map(groupMembers => groupMembers.filter(member => member.groupId === groupId))
        );
    return combineLatest(members$, usersSelector(store), (members, users) => {
...
export function memberSelector(store: Store<any>): Observable<Member> {
    const member$ = membersSelector(store).pipe(
        map(members => members.active)
    );
    const userId$ = member$.pipe(
        map(member => member.userId),
        distinctUntilChanged()
    );
    const user$ = userId$.pipe(
        mergeMap(id => userIdSelector(store, id))
    );
    return combineLatest(member$, user$,
        (member, user) => build(Member, member.base, { user }));
}

export function activeMemberIdSelector(store: Store<any>): Observable<number> {
    return memberSelector(store).pipe(
        map(member => member.userId),
        distinctUntilChanged()
    );
}
...
export function userGroupsSelector(store: Store<any>): Observable<Member[]> {
    ...
    return userId$.pipe(
        mergeMap(id => userMembershipsSelector(store, id))
    );
}
...
export function userMembershipsSelector(store: Store<any>, userId: number): Observable<Member[]> {
    return membersSelector(store)
        .pipe(
            map(members => members.toArray().filter(member => member.userId === userId))
        );
}
